...
PLATFORM_COMMON_DIR := $(TOPDIR)/target/linux/$(LICHEE_IC)-common
...
LINUX_TARGET_CONFIG := $(call find_kernel_config,$(LICHEE_TOP_DIR)/device/config/chips/$(LICHEE_IC)/configs/$(LICHEE_BOARD)/linux-$(KERNEL_PATCHVER))
ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
...
define BuildTargets/DumpCurrent
  ...
  dumpinfo:
	...
	$(if $(SUBTARGET),,$(if $(DEFAULT_SUBTARGET), echo 'Default-Subtarget: $(DEFAULT_SUBTARGET)'; )) \
	 ...
	 $(DUMPINFO)
	$(if $(CUR_SUBTARGET),$(SUBMAKE) -r --no-print-directory -C ../generic/image -s DUMP=1 SUBTARGET=$(CUR_SUBTARGET))
...
ifeq ($(TARGET_BUILD),1)
  ...
  ifneq ($(CONFIG_SKIP_KERNEL_BUILD),y)
    BuildTarget?=$(BuildKernel)
  else
    BuildTarget?=$(BuildSkipKernel)
  endif
endif
