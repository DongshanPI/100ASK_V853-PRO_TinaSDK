...
include $(LICHEE_TOP_DIR)/openwrt/build/tinahooks_toplevel.mk
export PATH:=$(strip $(TARGET_EXTERNAL_PATH)):$(subst $(space),:,$(filter-out $(subst :,$(space),$(TARGET_EXTERNAL_PATH)),$(subst :,$(space),$(PATH))))
...
tinahooks_prepare-tmpinfo: tinahooks/createtargetoutdir tinahooks/target/linux FORCE
	$(call TinaHooks/PrepareTmpdir)
	$(call TinaHooks/PrepareStagingdir)

prepare-tmpinfo: tinahooks_prepare-tmpinfo FORCE
	@+$(MAKE) -r -s staging_dir/host/.prereq-build $(PREP_MK)
	mkdir -p tmp/info
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f include/scan.mk SCAN_TARGET="packageinfo" SCAN_DIR="package" SCAN_NAME="package" SCAN_DEPTH=8 SCAN_EXTRA=""
...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	$(TinaHooks/CopyFromDefconfig)
	$< --allnoconfig Config.in;
	$(TinaHooks/CopyToDefconfig)
...
distclean:
	...
	rm -f target/linux  tmp  staging_dir
	rm -rf $(TARGET_OUT_DIR)/tmp \
		$(TARGET_OUT_DIR)/staging_dir \
		$(TARGET_OUT_DIR)/build_dir \
		$(TARGET_OUT_DIR)/logs \
		$(TARGET_OUT_DIR)/extra
