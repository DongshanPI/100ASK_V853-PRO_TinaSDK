...
ifneq ($(and $(LICHEE_TOP_DIR), $(LICHEE_IC), $(LICHEE_BOARD)),)
-include $(LICHEE_TOP_DIR)/openwrt/target/$(LICHEE_IC)/generic/BoardRules_generic.mk
-include $(LICHEE_TOP_DIR)/openwrt/target/$(LICHEE_IC)/$(LICHEE_IC)-common/BoardRules_common.mk
-include $(LICHEE_TOP_DIR)/openwrt/target/$(LICHEE_IC)/$(LICHEE_IC)-$(LICHEE_BOARD)/BoardRules.mk
else
$(error "Failed to include BoardRules.mk")
endif

include $(LICHEE_TOP_DIR)/openwrt/build/tinahooks_rules.mk
...
DL_DIR:=$(if $(call qstrip,$(CONFIG_DOWNLOAD_FOLDER)),$(call qstrip,$(CONFIG_DOWNLOAD_FOLDER)),$(LICHEE_TOP_DIR)/openwrt/dl)
#OUTPUT_DIR:=$(if $(call qstrip,$(CONFIG_BINARY_FOLDER)),$(call qstrip,$(CONFIG_BINARY_FOLDER)),$(TOPDIR)/bin)
OUTPUT_DIR:=$(if $(call qstrip,$(CONFIG_BINARY_FOLDER)),$(call qstrip,$(CONFIG_BINARY_FOLDER)),$(TARGET_OUT_DIR)/extra)
BIN_DIR:=$(OUTPUT_DIR)/targets/$(BOARD)/$(SUBTARGET)
INCLUDE_DIR:=$(TOPDIR)/include
SCRIPT_DIR:=$(TOPDIR)/scripts
#BUILD_DIR_BASE:=$(TOPDIR)/build_dir
BUILD_DIR_BASE:=$(TARGET_OUT_DIR)/build_dir
ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
  ...
  GCCV:=$(call qstrip,$(CONFIG_TOOLCHAIN_GCC_VERSION))
  LIBC:=$(call qstrip,$(CONFIG_LIBC))
  LIBC_VER:=$(call qstrip,$(CONFIG_TOOLCHAIN_LIBC_VERSION))
  TOOLCHAIN_NAME:=$(call qstrip,$(CONFIG_TARGET_NAME))
...
BUILD_LOG_DIR:=$(if $(call qstrip,$(CONFIG_BUILD_LOG_DIR)),$(call qstrip,$(CONFIG_BUILD_LOG_DIR)),$(TARGET_OUT_DIR)/logs)
  ...
  ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
    ...
    ifeq ($(CONFIG_NATIVE_TOOLCHAIN),)
      ...
      ifeq ($(CONFIG_COMPLILE_KERNEL64_USER32),y)
        KERNEL_TOOLCHAIN:=$(if $(call qstrip,$(CONFIG_KERNEL64_TOOLCHAIN)),$(call qstrip,$(CONFIG_KERNEL64_TOOLCHAIN)),/error)
        KERNEL_CROSS:=$(if $(call qstrip,$(CONFIG_KERNEL64_CROSS)),$(call qstrip,$(CONFIG_KERNEL64_CROSS)),error-)
        KERNEL_CC:=$(KERNEL_CROSS)gcc
        TARGET_PATH:=$(TOOLCHAIN_DIR)/bin:$(TARGET_PATH):$(KERNEL_TOOLCHAIN)/bin
      else
        TARGET_PATH:=$(TOOLCHAIN_DIR)/bin:$(TARGET_PATH)
      endif
    endif
