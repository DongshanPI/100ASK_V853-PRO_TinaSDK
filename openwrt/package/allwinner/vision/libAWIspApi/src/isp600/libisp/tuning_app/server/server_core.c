...
void *sock_handle_preview_vencode_thread(void *params)
{
	...
	struct sEncppSharpParamStatic encpp_static_sharp_cfg;
	struct sEncppSharpParamDynamic encpp_dynamic_sharp_cfg;
	...
	while (1) {
		...
		if (SHOULD_CLOSE_SOCK(ret)) {
			...
			if (encode_init == 1 && (type == SOCK_CMD_VENCODE_PPSSPS || type == SOCK_CMD_VENCODE_STREAM || type == SOCK_CMD_VENCODE_ENCPP_YUV)) {
				...
				SetEnableSharpEncoder(&encode_param, encpp_en);
				Set3DFliterParamEncoder(&encode_param, &encoder_3d_param);
				Set2DFliterParamEncoder(&encode_param, &encoder_2d_param);
				if (encpp_en) {
					isp_get_encpp_cfg(isp_sel, ISP_CTRL_ENCPP_STATIC_CFG, &encpp_static_sharp_cfg);
					isp_get_encpp_cfg(isp_sel, ISP_CTRL_ENCPP_DYNAMIC_CFG, &encpp_dynamic_sharp_cfg);
					encpp_sharp_param.pStaticParam = &encpp_static_sharp_cfg;
					encpp_sharp_param.pDynamicParam = &encpp_dynamic_sharp_cfg;
					SetSharpParamEncoder(&encode_param, &encpp_sharp_param);
				}
