...
HW_S32 isp_tuning_get_cfg(struct hw_isp_device *isp, HW_U8 group_id, HW_U32 cfg_ids, void *cfg_data)
{
	...
	case HW_ISP_CFG_3A: /* isp_3a_param */
		if (cfg_ids & HW_ISP_CFG_AE_PUB) /* isp_ae_pub */
		{
			...
			isp_ae_pub->hist0_sel = tuning->params.isp_3a_settings.ae_hist0_sel;
			isp_ae_pub->hist1_sel = tuning->params.isp_3a_settings.ae_hist1_sel;
		...
		if (cfg_ids & HW_ISP_CFG_WDR_SPLIT) /* isp_wdr_split  */
		{
			...
			memcpy(&isp_wdr_split->wdr_split_cfg[0], &tuning->params.isp_3a_settings.wdr_split_cfg[0],
		...
		if (cfg_ids & HW_ISP_CFG_WDR_STITCH) /* isp_wdr_stitch  */
		{
			...
			memcpy(&isp_wdr_cfg->value[0], &tuning->params.isp_3a_settings.wdr_comm_cfg[0],
	...
	case HW_ISP_CFG_TUNING: /* isp_tunning_param */
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_GTM) /* isp_tuning_gtm */
		{
			...
			isp_tuning_gtm->gtm_hist_sel = tuning->params.isp_tunning_settings.gtm_hist_sel;
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_CFA) /* isp_tuning_cfa */
		{
			...
			isp_tuning_cfa->grad_th = tuning->params.isp_tunning_settings.grad_th;
			isp_tuning_cfa->dir_v_th = tuning->params.isp_tunning_settings.dir_v_th;
			isp_tuning_cfa->dir_h_th = tuning->params.isp_tunning_settings.dir_h_th;
			isp_tuning_cfa->res_smth_high = tuning->params.isp_tunning_settings.res_smth_high;
			isp_tuning_cfa->res_smth_low = tuning->params.isp_tunning_settings.res_smth_low;
			isp_tuning_cfa->res_high_th = tuning->params.isp_tunning_settings.res_high_th;
			isp_tuning_cfa->res_low_th = tuning->params.isp_tunning_settings.res_low_th;
			isp_tuning_cfa->res_dir_a = tuning->params.isp_tunning_settings.res_dir_a;
			isp_tuning_cfa->res_dir_d = tuning->params.isp_tunning_settings.res_dir_d;
			isp_tuning_cfa->res_dir_v = tuning->params.isp_tunning_settings.res_dir_v;
			isp_tuning_cfa->res_dir_h = tuning->params.isp_tunning_settings.res_dir_h;
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_CCM_LOW) /* isp_ccm_low */
		{
			...
			isp_ccm_low->temperature = tuning->params.isp_tunning_settings.ccm_trig_cfg[0];
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_CCM_MID) /* isp_ccm_mid */
		{
			...
			isp_ccm_mid->temperature = tuning->params.isp_tunning_settings.ccm_trig_cfg[1];
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_CCM_HIGH) /* isp_ccm_high */
		{
			...
			isp_ccm_high->temperature = tuning->params.isp_tunning_settings.ccm_trig_cfg[2];
		...
		{
			memcpy(data_ptr, tuning->params.isp_tunning_settings.denoise_comm_cfg, sizeof(struct isp_tuning_bdnf_comm_cfg));
		...
		{
			memcpy(data_ptr, tuning->params.isp_tunning_settings.tdf_comm_cfg, sizeof(struct isp_tuning_tdnf_comm_cfg));
		...
		{
			memcpy(data_ptr, tuning->params.isp_tunning_settings.sharp_comm_cfg, sizeof(struct isp_tuning_sharp_comm_cfg));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_DPC)
		{
			...
			isp_tuning_dpc->dpc_remove_mode = tuning->params.isp_tunning_settings.dpc_remove_mode;
			isp_tuning_dpc->dpc_sup_twinkle_en = tuning->params.isp_tunning_settings.dpc_sup_twinkle_en;
		...
		{
			memcpy(data_ptr, tuning->params.isp_tunning_settings.encpp_sharp_comm_cfg, sizeof(struct isp_tuning_encpp_sharp_comm_cfg));
	...
	case HW_ISP_CFG_TUNING_TABLES: /* isp tuning tables */
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_BDNF) /* isp_tuning_bdnf */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(isp_tuning_bdnf->thres[i].lp0_thres, tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp0_th, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
				memcpy(isp_tuning_bdnf->thres[i].lp1_thres, tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp1_th, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
				memcpy(isp_tuning_bdnf->thres[i].lp2_thres, tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp2_th, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
				memcpy(isp_tuning_bdnf->thres[i].lp3_thres, tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp3_th, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
			}
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_TDNF) /* isp_tuning_tdnf */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(&isp_tuning_tdnf->thres[i][0], tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d3d_flt0_thr_vc, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
			}
			memcpy(isp_tuning_tdnf->df_shape, tuning->params.isp_tunning_settings.isp_tdnf_df_shape, sizeof(isp_tuning_tdnf->df_shape));
			memcpy(isp_tuning_tdnf->ratio_amp, tuning->params.isp_tunning_settings.isp_tdnf_ratio_amp, sizeof(isp_tuning_tdnf->ratio_amp));
			memcpy(isp_tuning_tdnf->k_dlt_bk, tuning->params.isp_tunning_settings.isp_tdnf_k_dlt_bk, sizeof(isp_tuning_tdnf->k_dlt_bk));
			memcpy(isp_tuning_tdnf->ct_rt_bk, tuning->params.isp_tunning_settings.isp_tdnf_ct_rt_bk, sizeof(isp_tuning_tdnf->ct_rt_bk));
			memcpy(isp_tuning_tdnf->dtc_hf_bk, tuning->params.isp_tunning_settings.isp_tdnf_dtc_hf_bk, sizeof(isp_tuning_tdnf->dtc_hf_bk));
			memcpy(isp_tuning_tdnf->dtc_mf_bk, tuning->params.isp_tunning_settings.isp_tdnf_dtc_mf_bk, sizeof(isp_tuning_tdnf->dtc_mf_bk));
			memcpy(isp_tuning_tdnf->lay0_d2d0_rt_br, tuning->params.isp_tunning_settings.isp_tdnf_lay0_d2d0_rt_br, sizeof(isp_tuning_tdnf->lay0_d2d0_rt_br));
			memcpy(isp_tuning_tdnf->lay1_d2d0_rt_br, tuning->params.isp_tunning_settings.isp_tdnf_lay1_d2d0_rt_br, sizeof(isp_tuning_tdnf->lay1_d2d0_rt_br));
			memcpy(isp_tuning_tdnf->lay0_nrd_rt_br, tuning->params.isp_tunning_settings.isp_tdnf_lay0_nrd_rt_br, sizeof(isp_tuning_tdnf->lay0_nrd_rt_br));
			memcpy(isp_tuning_tdnf->lay1_nrd_rt_br, tuning->params.isp_tunning_settings.isp_tdnf_lay1_nrd_rt_br, sizeof(isp_tuning_tdnf->lay1_nrd_rt_br));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_SHARP) /* isp_tuning_sharp */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(&isp_tuning_sharp->edge_lum[i][0], tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_edge_lum, ISP_REG_TBL_LENGTH * sizeof(HW_U16));
			}
			memcpy(isp_tuning_sharp->ss_val, tuning->params.isp_tunning_settings.isp_sharp_ss_value, sizeof(isp_tuning_sharp->ss_val));
			memcpy(isp_tuning_sharp->ls_val, tuning->params.isp_tunning_settings.isp_sharp_ls_value, sizeof(isp_tuning_sharp->ls_val));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_PLTM_TBL) /* isp_tuning_pltm_table */
		{
			...
			memcpy(isp_tuning_pltm->stat_gd_cv, tuning->params.isp_tunning_settings.isp_pltm_stat_gd_cv,
				sizeof(isp_tuning_pltm->stat_gd_cv));
			memcpy(isp_tuning_pltm->df_cv, tuning->params.isp_tunning_settings.isp_pltm_df_cv,
				sizeof(isp_tuning_pltm->df_cv));
			memcpy(isp_tuning_pltm->lum_map_cv, tuning->params.isp_tunning_settings.isp_pltm_lum_map_cv,
				sizeof(isp_tuning_pltm->lum_map_cv));
			memcpy(isp_tuning_pltm->gtm_tbl, tuning->params.isp_tunning_settings.isp_pltm_gtm_tbl,
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_WDR) /* isp_tuning_wdr_tbl */
		{
			...
			memcpy(isp_tuning_wdr->wdr_de_purpl_hsv_tbl, tuning->params.isp_tunning_settings.isp_wdr_de_purpl_hsv_tbl,
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_ENCPP_SHARP) /* isp_tuning_encpp_sharp */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(&isp_tuning_encpp_sharp->edge_lum[i][0], tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_edge_lum, ISP_REG_TBL_LENGTH * sizeof(HW_U16));
			}
			memcpy(isp_tuning_encpp_sharp->ss_val, tuning->params.isp_tunning_settings.encpp_sharp_ss_value, sizeof(isp_tuning_encpp_sharp->ss_val));
			memcpy(isp_tuning_encpp_sharp->ls_val, tuning->params.isp_tunning_settings.encpp_sharp_ls_value, sizeof(isp_tuning_encpp_sharp->ls_val));
			memcpy(isp_tuning_encpp_sharp->hsv, tuning->params.isp_tunning_settings.encpp_sharp_hsv, sizeof(isp_tuning_encpp_sharp->hsv));
			/* offset */
			data_ptr += sizeof(struct isp_tuning_sharp_table_cfg);
			ret += sizeof(struct isp_tuning_sharp_table_cfg);
		}
	...
	case HW_ISP_CFG_DYNAMIC: /* isp_dynamic_param */
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_SHARP) /* isp_dynamic_sharp */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(isp_dynamic_sharp->tuning_ss_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[0],
				        sizeof(isp_dynamic_sharp->tuning_ss_cfg[i].value));
				memcpy(isp_dynamic_sharp->tuning_ls_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[ISP_SHARP_LS_NS_LW],
				        sizeof(isp_dynamic_sharp->tuning_ls_cfg[i].value));
				memcpy(isp_dynamic_sharp->tuning_hfr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[ISP_SHARP_HFR_SMTH_RATIO],
				        sizeof(isp_dynamic_sharp->tuning_hfr_cfg[i].value));
				memcpy(isp_dynamic_sharp->tuning_comm_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[ISP_SHARP_DIR_SMTH0],
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_DENOISE) /* isp_dynamic_denoise */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(isp_dynamic_denoise->tuning_dnr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].denoise_cfg[0],
					sizeof(isp_dynamic_denoise->tuning_dnr_cfg[i].value));
				isp_dynamic_denoise->tuning_dnr_cfg[i].color_denoise = tuning->params.isp_iso_settings.isp_dynamic_cfg[i].color_denoise;
				memcpy(isp_dynamic_denoise->tuning_dtc_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].denoise_cfg[ISP_DENOISE_HF_SMTH_RATIO],
					sizeof(isp_dynamic_denoise->tuning_dtc_cfg[i].value));
				memcpy(isp_dynamic_denoise->tuning_wdr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].denoise_cfg[ISP_DENOISE_LYR0_DNR_LM_AMP],
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_CEM) /* isp_dynamic_cem */
		{
			...
			int i = 0;
			isp_dynamic_cem->trigger = tuning->params.isp_iso_settings.triger.cem_triger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
				memcpy(isp_dynamic_cem->tuning_cfg[i].value, tuning->params.isp_iso_settings.isp_dynamic_cfg[i].cem_cfg,
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_TDF) /* isp_dynamic_tdf */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(isp_dynamic_tdf->tuning_dnr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[0],
					sizeof(isp_dynamic_tdf->tuning_dnr_cfg[i].value));
				memcpy(isp_dynamic_tdf->tuning_mtd_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[ISP_TDF_DIFF_INTRA_SENS],
					sizeof(isp_dynamic_tdf->tuning_mtd_cfg[i].value));
				memcpy(isp_dynamic_tdf->tuning_dtc_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[ISP_TDF_DTC_HF_COR],
					sizeof(isp_dynamic_tdf->tuning_dtc_cfg[i].value));
				memcpy(isp_dynamic_tdf->tuning_srd_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[ISP_TDF_D2D0_CNR_STREN],
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_CFA) /* isp_dynamic_cfa */
		{
			...
			int i = 0;
			isp_dynamic_cfa->trigger = tuning->params.isp_iso_settings.triger.cfa_triger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
				memcpy(isp_dynamic_cfa->tuning_cfg[i].value, tuning->params.isp_iso_settings.isp_dynamic_cfg[i].cfa_cfg,
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_ENCPP_SHARP) /* isp_dynamic_encpp_sharp */
		{
			...
			int i = 0;
			isp_dynamic_encpp_sharp->trigger = tuning->params.isp_iso_settings.triger.encpp_sharp_triger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(isp_dynamic_encpp_sharp->tuning_ss_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[0],
				        sizeof(isp_dynamic_encpp_sharp->tuning_ss_cfg[i].value));
				memcpy(isp_dynamic_encpp_sharp->tuning_ls_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[ENCPP_SHARP_LS_NS_LW],
				        sizeof(isp_dynamic_encpp_sharp->tuning_ls_cfg[i].value));
				memcpy(isp_dynamic_encpp_sharp->tuning_hfr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[ENCPP_SHARP_HFR_SMTH_RATIO],
				        sizeof(isp_dynamic_encpp_sharp->tuning_hfr_cfg[i].value));
				memcpy(isp_dynamic_encpp_sharp->tuning_comm_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[ENCPP_SHARP_DIR_SMTH0],
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_ENCODER_DENOISE) /* isp_dynamic_encoder_denoise */
		{
			...
			int i = 0;
			isp_dynamic_encoder_denoise->trigger = tuning->params.isp_iso_settings.triger.encoder_denoise_triger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(isp_dynamic_encoder_denoise->tuning_3dnr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encoder_denoise_cfg[0],
				        sizeof(isp_dynamic_encoder_denoise->tuning_3dnr_cfg[i].value));
				memcpy(isp_dynamic_encoder_denoise->tuning_2dnr_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encoder_denoise_cfg[ENCODER_DENOISE_2D_FILT_STREN_UV],
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_WDR) /* isp_dynamic_wdr */
		{
			...
			int i = 0;
			isp_dynamic_wdr->trigger = tuning->params.isp_iso_settings.triger.wdr_cfg_triger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(isp_dynamic_wdr->tuning_lm_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].wdr_cfg[0],
						sizeof(isp_dynamic_wdr->tuning_lm_cfg[i].value));
				memcpy(isp_dynamic_wdr->tuning_ms_cfg[i].value, &tuning->params.isp_iso_settings.isp_dynamic_cfg[i].wdr_cfg[WDR_CMP_MS_LTH],
...
HW_S32 isp_tuning_set_cfg(struct hw_isp_device *isp, HW_U8 group_id, HW_U32 cfg_ids, void *cfg_data)
{
	...
	case HW_ISP_CFG_3A: /* isp_3a_param */
		if (cfg_ids & HW_ISP_CFG_AE_PUB) /* isp_ae_pub */
		{
			...
			tuning->params.isp_3a_settings.ae_hist0_sel = isp_ae_pub->hist0_sel;
			tuning->params.isp_3a_settings.ae_hist1_sel = isp_ae_pub->hist1_sel;
		...
		{
			struct isp_wdr_split_cfg *isp_wdr_split = (struct isp_wdr_split_cfg *)data_ptr;
			memcpy(&tuning->params.isp_3a_settings.wdr_split_cfg[0], &isp_wdr_split->wdr_split_cfg[0],
				sizeof(isp_wdr_split->wdr_split_cfg));
		...
		{
			struct isp_wdr_comm_cfg *isp_wdr_cfg = (struct isp_wdr_comm_cfg *)data_ptr;
			memcpy(&tuning->params.isp_3a_settings.wdr_comm_cfg[0], &isp_wdr_cfg->value[0],
				sizeof(isp_wdr_cfg->value));
	...
	case HW_ISP_CFG_TUNING: /* isp_tunning_param */
		...
		{
			struct isp_tuning_gtm_cfg *isp_tuning_gtm = (struct isp_tuning_gtm_cfg *)data_ptr;
			tuning->params.isp_tunning_settings.gtm_hist_sel = isp_tuning_gtm->gtm_hist_sel;
		...
		{
			struct isp_tuning_cfa_cfg *isp_tuning_cfa = (struct isp_tuning_cfa_cfg *)data_ptr;
			tuning->params.isp_tunning_settings.grad_th = isp_tuning_cfa->grad_th;
			tuning->params.isp_tunning_settings.dir_v_th = isp_tuning_cfa->dir_v_th;
			tuning->params.isp_tunning_settings.dir_h_th = isp_tuning_cfa->dir_h_th;
			tuning->params.isp_tunning_settings.res_smth_high = isp_tuning_cfa->res_smth_high;
			tuning->params.isp_tunning_settings.res_smth_low = isp_tuning_cfa->res_smth_low;
			tuning->params.isp_tunning_settings.res_high_th = isp_tuning_cfa->res_high_th;
			tuning->params.isp_tunning_settings.res_low_th = isp_tuning_cfa->res_low_th;
			tuning->params.isp_tunning_settings.res_dir_a = isp_tuning_cfa->res_dir_a;
			tuning->params.isp_tunning_settings.res_dir_d = isp_tuning_cfa->res_dir_d;
			tuning->params.isp_tunning_settings.res_dir_v = isp_tuning_cfa->res_dir_v;
			tuning->params.isp_tunning_settings.res_dir_h = isp_tuning_cfa->res_dir_h;
		...
		{
			struct isp_tuning_ccm_cfg *isp_ccm_low = (struct isp_tuning_ccm_cfg *)data_ptr;
			tuning->params.isp_tunning_settings.ccm_trig_cfg[0] = isp_ccm_low->temperature;
		...
		{
			struct isp_tuning_ccm_cfg *isp_ccm_mid = (struct isp_tuning_ccm_cfg *)data_ptr;
			tuning->params.isp_tunning_settings.ccm_trig_cfg[1] = isp_ccm_mid->temperature;
		...
		{
			struct isp_tuning_ccm_cfg *isp_ccm_high = (struct isp_tuning_ccm_cfg *)data_ptr;
			tuning->params.isp_tunning_settings.ccm_trig_cfg[2] = isp_ccm_high->temperature;
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_BDNF_COMM)
		{
			memcpy(tuning->params.isp_tunning_settings.denoise_comm_cfg, data_ptr, sizeof(struct isp_tuning_bdnf_comm_cfg));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_TDNF_COMM)
		{
			memcpy(tuning->params.isp_tunning_settings.tdf_comm_cfg, data_ptr, sizeof(struct isp_tuning_tdnf_comm_cfg));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_SHARP_COMM)
		{
			memcpy(tuning->params.isp_tunning_settings.sharp_comm_cfg, data_ptr, sizeof(struct isp_tuning_sharp_comm_cfg));
		...
		{
			struct isp_tuning_dpc_cfg *isp_tuning_dpc = (struct isp_tuning_dpc_cfg *)data_ptr;
			tuning->params.isp_tunning_settings.dpc_remove_mode = isp_tuning_dpc->dpc_remove_mode;
			tuning->params.isp_tunning_settings.dpc_sup_twinkle_en = isp_tuning_dpc->dpc_sup_twinkle_en;
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_ENCPP_SHARP_COMM)
		{
			memcpy(tuning->params.isp_tunning_settings.encpp_sharp_comm_cfg, data_ptr, sizeof(struct isp_tuning_encpp_sharp_comm_cfg));
	...
	case HW_ISP_CFG_TUNING_TABLES: /* isp tuning tables*/
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_BDNF) /* isp_tuning_bdnf */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp0_th, isp_tuning_bdnf->thres[i].lp0_thres, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp1_th, isp_tuning_bdnf->thres[i].lp1_thres, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp2_th, isp_tuning_bdnf->thres[i].lp2_thres, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d2d_lp3_th, isp_tuning_bdnf->thres[i].lp3_thres, ISP_REG_TBL_LENGTH * sizeof(HW_S16));
			}
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_TDNF) /* isp_tuning_tdnf */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].d3d_flt0_thr_vc, &isp_tuning_tdnf->thres[i][0], ISP_REG_TBL_LENGTH * sizeof(HW_S16));
			}
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_df_shape, isp_tuning_tdnf->df_shape, sizeof(isp_tuning_tdnf->df_shape));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_ratio_amp, isp_tuning_tdnf->ratio_amp, sizeof(isp_tuning_tdnf->ratio_amp));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_k_dlt_bk, isp_tuning_tdnf->k_dlt_bk, sizeof(isp_tuning_tdnf->k_dlt_bk));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_ct_rt_bk, isp_tuning_tdnf->ct_rt_bk, sizeof(isp_tuning_tdnf->ct_rt_bk));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_dtc_hf_bk, isp_tuning_tdnf->dtc_hf_bk, sizeof(isp_tuning_tdnf->dtc_hf_bk));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_dtc_mf_bk, isp_tuning_tdnf->dtc_mf_bk, sizeof(isp_tuning_tdnf->dtc_mf_bk));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_lay0_d2d0_rt_br, isp_tuning_tdnf->lay0_d2d0_rt_br, sizeof(isp_tuning_tdnf->lay0_d2d0_rt_br));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_lay1_d2d0_rt_br, isp_tuning_tdnf->lay1_d2d0_rt_br, sizeof(isp_tuning_tdnf->lay1_d2d0_rt_br));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_lay0_nrd_rt_br, isp_tuning_tdnf->lay0_nrd_rt_br, sizeof(isp_tuning_tdnf->lay0_nrd_rt_br));
			memcpy(tuning->params.isp_tunning_settings.isp_tdnf_lay1_nrd_rt_br, isp_tuning_tdnf->lay1_nrd_rt_br, sizeof(isp_tuning_tdnf->lay1_nrd_rt_br));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_SHARP) /* isp_tuning_sharp */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_edge_lum, &isp_tuning_sharp->edge_lum[i][0], ISP_REG_TBL_LENGTH * sizeof(HW_U16));
			}
			memcpy(tuning->params.isp_tunning_settings.isp_sharp_ss_value, isp_tuning_sharp->ss_val, sizeof(isp_tuning_sharp->ss_val));
			memcpy(tuning->params.isp_tunning_settings.isp_sharp_ls_value, isp_tuning_sharp->ls_val, sizeof(isp_tuning_sharp->ss_val));
		...
		{
			struct isp_tuning_pltm_table_cfg *isp_tuning_pltm = (struct isp_tuning_pltm_table_cfg *)data_ptr;
			memcpy(tuning->params.isp_tunning_settings.isp_pltm_stat_gd_cv, isp_tuning_pltm->stat_gd_cv,
							sizeof(isp_tuning_pltm->stat_gd_cv));
			memcpy(tuning->params.isp_tunning_settings.isp_pltm_df_cv, isp_tuning_pltm->df_cv,
							sizeof(isp_tuning_pltm->df_cv));
			memcpy(tuning->params.isp_tunning_settings.isp_pltm_lum_map_cv, isp_tuning_pltm->lum_map_cv,
							sizeof(isp_tuning_pltm->lum_map_cv));
			memcpy(tuning->params.isp_tunning_settings.isp_pltm_gtm_tbl, isp_tuning_pltm->gtm_tbl,
							sizeof(isp_tuning_pltm->gtm_tbl));
		...
		{
			struct isp_tuning_wdr_table_cfg *isp_tuning_wdr = (struct isp_tuning_wdr_table_cfg *)data_ptr;
			memcpy(tuning->params.isp_tunning_settings.isp_wdr_de_purpl_hsv_tbl, isp_tuning_wdr->wdr_de_purpl_hsv_tbl,
							sizeof(isp_tuning_wdr->wdr_de_purpl_hsv_tbl));
		...
		if (cfg_ids & HW_ISP_CFG_TUNING_ENCPP_SHARP) /* isp_tuning_sharp */
		{
			...
			int i = 0;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++) {
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_edge_lum, &isp_tuning_encpp_sharp->edge_lum[i][0], ISP_REG_TBL_LENGTH * sizeof(HW_U16));
			}
			memcpy(tuning->params.isp_tunning_settings.encpp_sharp_ss_value, isp_tuning_encpp_sharp->ss_val, sizeof(isp_tuning_encpp_sharp->ss_val));
			memcpy(tuning->params.isp_tunning_settings.encpp_sharp_ls_value, isp_tuning_encpp_sharp->ls_val, sizeof(isp_tuning_encpp_sharp->ss_val));
			memcpy(tuning->params.isp_tunning_settings.encpp_sharp_hsv, isp_tuning_encpp_sharp->hsv, sizeof(isp_tuning_encpp_sharp->hsv));
			/* offset */
			data_ptr += sizeof(struct isp_tuning_sharp_table_cfg);
			ret += sizeof(struct isp_tuning_sharp_table_cfg);
		}
	...
	case HW_ISP_CFG_DYNAMIC: /* isp_dynamic_param */
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_SHARP) /* isp_dynamic_sharp */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[0], isp_dynamic_sharp->tuning_ss_cfg[i].value,
				        sizeof(isp_dynamic_sharp->tuning_ss_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[ISP_SHARP_LS_NS_LW], isp_dynamic_sharp->tuning_ls_cfg[i].value,
				        sizeof(isp_dynamic_sharp->tuning_ls_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[ISP_SHARP_HFR_SMTH_RATIO], isp_dynamic_sharp->tuning_hfr_cfg[i].value,
				        sizeof(isp_dynamic_sharp->tuning_hfr_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].sharp_cfg[ISP_SHARP_DIR_SMTH0], isp_dynamic_sharp->tuning_comm_cfg[i].value,
				        sizeof(isp_dynamic_sharp->tuning_comm_cfg[i].value));
			}
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_DENOISE) /* isp_dynamic_denoise */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].denoise_cfg[0], isp_dynamic_denoise->tuning_dnr_cfg[i].value,
					sizeof(isp_dynamic_denoise->tuning_dnr_cfg[i].value));
				tuning->params.isp_iso_settings.isp_dynamic_cfg[i].color_denoise = isp_dynamic_denoise->tuning_dnr_cfg[i].color_denoise;
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].denoise_cfg[ISP_DENOISE_HF_SMTH_RATIO], isp_dynamic_denoise->tuning_dtc_cfg[i].value,
					sizeof(isp_dynamic_denoise->tuning_dtc_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].denoise_cfg[ISP_DENOISE_LYR0_DNR_LM_AMP], isp_dynamic_denoise->tuning_wdr_cfg[i].value,
					sizeof(isp_dynamic_denoise->tuning_wdr_cfg[i].value));
			}
		...
		{	struct isp_dynamic_cem_cfg *isp_dynamic_cem = (struct isp_dynamic_cem_cfg *)data_ptr;
			int i = 0;
			tuning->params.isp_iso_settings.triger.cem_triger = (enum isp_triger_type)isp_dynamic_cem->trigger ;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].cem_cfg, isp_dynamic_cem->tuning_cfg[i].value,
					sizeof(isp_dynamic_cem->tuning_cfg[i].value));
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_TDF) /* isp_dynamic_tdf */
		{
			...
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[0], isp_dynamic_tdf->tuning_dnr_cfg[i].value,
					sizeof(isp_dynamic_tdf->tuning_dnr_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[ISP_TDF_DIFF_INTRA_SENS], isp_dynamic_tdf->tuning_mtd_cfg[i].value,
					sizeof(isp_dynamic_tdf->tuning_mtd_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[ISP_TDF_DTC_HF_COR], isp_dynamic_tdf->tuning_dtc_cfg[i].value,
					sizeof(isp_dynamic_tdf->tuning_dtc_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].tdf_cfg[ISP_TDF_D2D0_CNR_STREN], isp_dynamic_tdf->tuning_srd_cfg[i].value,
					sizeof(isp_dynamic_tdf->tuning_srd_cfg[i].value));
			}
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_CFA) /* isp_dynamic_cfa */
		{
			struct isp_dynamic_cfa_cfg *isp_dynamic_cfa = (struct isp_dynamic_cfa_cfg *)data_ptr;
			int i = 0;
			tuning->params.isp_iso_settings.triger.cfa_triger = (enum isp_triger_type)isp_dynamic_cfa->trigger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
				memcpy(tuning->params.isp_iso_settings.isp_dynamic_cfg[i].cfa_cfg, isp_dynamic_cfa->tuning_cfg[i].value,
					sizeof(isp_dynamic_cfa->tuning_cfg[i].value));
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_ENCPP_SHARP) /* isp_dynamic_encpp_sharp */
		{
			struct isp_dynamic_encpp_sharp_cfg *isp_dynamic_encpp_sharp = (struct isp_dynamic_encpp_sharp_cfg *)data_ptr;
			int i = 0;
			tuning->params.isp_iso_settings.triger.encpp_sharp_triger = (enum isp_triger_type)isp_dynamic_encpp_sharp->trigger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[0], isp_dynamic_encpp_sharp->tuning_ss_cfg[i].value,
				        sizeof(isp_dynamic_encpp_sharp->tuning_ss_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[ENCPP_SHARP_LS_NS_LW], isp_dynamic_encpp_sharp->tuning_ls_cfg[i].value,
				        sizeof(isp_dynamic_encpp_sharp->tuning_ls_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[ENCPP_SHARP_HFR_SMTH_RATIO], isp_dynamic_encpp_sharp->tuning_hfr_cfg[i].value,
				        sizeof(isp_dynamic_encpp_sharp->tuning_hfr_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encpp_sharp_cfg[ENCPP_SHARP_DIR_SMTH0], isp_dynamic_encpp_sharp->tuning_comm_cfg[i].value,
				        sizeof(isp_dynamic_encpp_sharp->tuning_comm_cfg[i].value));
			}
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_ENCODER_DENOISE) /* isp_dynamic_encoder_denoise */
		{
			struct isp_dynamic_encoder_denoise_cfg *isp_dynamic_encoder_denoise = (struct isp_dynamic_encoder_denoise_cfg *)data_ptr;
			int i = 0;
			tuning->params.isp_iso_settings.triger.encoder_denoise_triger = (enum isp_triger_type)isp_dynamic_encoder_denoise->trigger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encoder_denoise_cfg[0], isp_dynamic_encoder_denoise->tuning_3dnr_cfg[i].value,
						sizeof(isp_dynamic_encoder_denoise->tuning_3dnr_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].encoder_denoise_cfg[ENCODER_DENOISE_2D_FILT_STREN_UV], isp_dynamic_encoder_denoise->tuning_2dnr_cfg[i].value,
						sizeof(isp_dynamic_encoder_denoise->tuning_2dnr_cfg[i].value));
			}
		...
		if (cfg_ids & HW_ISP_CFG_DYNAMIC_WDR) /* isp_dynamic_wdr */
		{
			struct isp_dynamic_wdr_cfg *isp_dynamic_wdr = (struct isp_dynamic_wdr_cfg *)data_ptr;
			int i = 0;
			tuning->params.isp_iso_settings.triger.wdr_cfg_triger = (enum isp_triger_type)isp_dynamic_wdr->trigger;
			for (i = 0; i < ISP_DYNAMIC_GROUP_COUNT; i++)
			{
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].wdr_cfg[0], isp_dynamic_wdr->tuning_lm_cfg[i].value,
						sizeof(isp_dynamic_wdr->tuning_lm_cfg[i].value));
				memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[i].wdr_cfg[WDR_CMP_MS_LTH], isp_dynamic_wdr->tuning_ms_cfg[i].value,
						sizeof(isp_dynamic_wdr->tuning_ms_cfg[i].value));
			}
