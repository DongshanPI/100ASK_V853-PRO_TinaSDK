...
int np_get_scene_config(NP_SCENE * scene, const char *scene_name)
{
	...
	printf("get uci config:%s\n", scene_name);
	printf("bootlock:%s", scene->cpu.bootlock);
	printf("roomage:%s", scene->cpu.roomage);
	printf("cpu_freq:%s", scene->cpu.cpu_freq);
	printf("cpu_freq_max:%s", scene->cpu.cpu_freq_max);
	printf("cpu_freq_min:%s", scene->cpu.cpu_freq_min);
	printf("cpu_gov:%s", scene->cpu.cpu_gov);
	printf("cpu_hot:%s", scene->cpu.cpu_hot);
	printf("cpu_online:%s", scene->cpu.cpu_online);

	printf("gpu_freq:%s", scene->gpu.gpu_freq);

	printf("dram_adaptive:%s", scene->dram.dram_adaptive);
	printf("dram_freq:%s", scene->dram.dram_freq);
	printf("dram_freq_max:%s", scene->dram.dram_freq_max);
	printf("dram_freq_min:%s", scene->dram.dram_freq_min);
...
static int setConfig(const char *path, const char *value)
{
	...
	printf("path:%s set value:%s", path, value);
...
static int GetGpuFreq(char *buf, int len)
{
	return -1;
}
