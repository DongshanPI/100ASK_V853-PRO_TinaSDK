一、简介
    tina 使用busybox init方式启动，首先调用执行/init（挂载文件系统，如/proc、/tmp、/sys
...
------------------------------------------------------------------------------------------
三、/init与/etc/init.d/rcS
    /init与/etc/init.d/rcS文件中存在很多平台共性的代码，避免系统充斥大量冗余代码,以及方便基础文件的维护和开发。
所以不允许在特定平台下自定义/init、/etc/init.d/rcS文件（必须使用tina/packge/busybox-init-base-files/files下
的/init、/etc/init.d/rcS）。
    如果需要添加平台特定配置（/init，/etc/init.d/rcS没有配置），可将其写到rc.preboot,rc.final中，参考第四节。
...
启动应用程序,该脚本将会被/etc/init.d/rcS最后调用执行。
